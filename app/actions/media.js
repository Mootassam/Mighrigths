...
const addTenantId = (tenant_id) => {
  return {
    type: actionTypes.TENANT_ID,
    tenant_id,
  };
};
...
export const onCurrentUser = (user_id, tenant_id) => (dispatch) => {
  dispatch(currentUser(user_id));
  dispatch(addTenantId(tenant_id));
};
...
export const onAddTenantId = (tenant_id) => (dispatch) => {
  dispatch(addTenantId(tenant_id));
};
